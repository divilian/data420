...
for i in range(1,len(t)):
    ...
    mouse_death_rate = mouse_death_factor * M[i-1] + kill_rate  # mice/month
    bat_death_rate = bat_death_factor * B[i-1] - nutrition_factor * kill_rate
        # bats/month
...
fig, ax = plt.subplots()
ax.plot(t,B,color="black",label="bats",linewidth=2)
ax.plot(t,M,color="brown",label="mice",linewidth=2,linestyle="dotted")
ax.set_ylim(0,M.max()+10)
